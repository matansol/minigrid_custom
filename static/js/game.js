...
//         // Hide the start button and show the game and info containers
...
//         const key = event.key;
//         const validKeys = ["ArrowLeft", "ArrowRight", "ArrowUp", "Space", "PageUp", "PageDown", "Tab", "Shift", "1", "2"];
...
document.addEventListener('DOMContentLoaded', (event) => {
    const socket = io.connect('http://' + document.domain + ':' + location.port);
    const startButton = document.getElementById('start-button');
    const ppoButton = document.getElementById('ppo-button');  // PPO button
    const gameContainer = document.getElementById('game-container');
    const infoContainer = document.getElementById('info-container');
    ...
    startButton.addEventListener('click', () => {
        ...
        // Hide the player name input and the start button, show the game and info containers
        playerNameInput.style.display = 'none';
        startButton.style.display = 'none';
        ppoButton.style.display = 'block';  // Show the PPO button
        gameContainer.style.display = 'flex';
        infoContainer.style.display = 'block';
    });
